        ...
        pub fn detectCycles(self: *Self) !CycleList {
            ...
            for (self.tracked_arcs.items) |arc_ptr| {
                ...
                if (!reachable.contains(arc_ptr) and arc_ptr.counters.strong_count.load(.monotonic) > 0) {
                    ...
                    // Create Arc from raw pointer - this wraps the pointer without incrementing refcount
                    // The existing strong references from the cycle keep the object alive
                    const temp = Arc(T){ .storage = .{ .ptr_with_tag = tagged.toUnsigned() } };
                    // Clone to properly increment refcount for the returned list
                    const arc_for_list = temp.clone();
                    // Release temp's implicit reference (it didn't actually increment on creation)
                    // This brings us back to the original refcount + 1 from clone()
                    temp.release();
