...
// All benchmark runs write timestamped markdown results under:
...
fn initMdPath() void {
    ...
    // Format: YYYYMMDD_HHMMSS (simplified from epoch)
...
fn run_arc_scaling_tables() !void {
    const allocator = getAllocator();
    // Note: 8+ thread rows temporarily disabled to keep runs shorter.
    // const threads_list_all = [_]usize{1,4,8,16};
    // const threads_list = [_]usize{ 1, 4, 8 }; // COMMENTED OUT: 8+ thread benchmarks
    const threads_list = [_]usize{ 1, 4 };
    ...
    var buf1: [768]u8 = undefined;
    var fbs1 = std.io.fixedBufferStream(&buf1);
    const wr1 = fbs1.writer();
    ...
    var buf4: [768]u8 = undefined;
    var fbs4 = std.io.fixedBufferStream(&buf4);
    const wr4 = fbs4.writer();
...
fn benchMultiDowngradeUpgradeType(
    ...
    var stats = Stats{};
    var shared = try ArcType.init(allocator, value);
    defer shared.release();
    const handles = try allocator.alloc(Thread, threads);
    defer allocator.free(handles);
    const ctxs = try allocator.alloc(Ctx, threads);
    defer allocator.free(ctxs);
    var k: usize = 0;
    while (k < threads) : (k += 1) {
        ctxs[k] = .{ .shared = &shared, .iterations = per_thread_iters };
        handles[k] = try Thread.spawn(.{}, Worker.run, .{&ctxs[k]});
    }
    var t = try Timer.start();
    for (handles) |h| h.join();
    const ns = t.read();
...
fn run_arc_downgrade_tables() !void {
    ...
    var s1 = std.io.fixedBufferStream(&b1);
    const w1 = s1.writer();
    try w1.print("### Arc - SVO Downgrade + Upgrade Throughput - (u32)\n", .{});
    try w1.print("| Thread(s) |  Iterations | ns/op (median) | Throughput (ops/s) |\n| --- | --- | --- | --- |\n", .{});
    for (threads_list) |tc| {
        const r = try benchMultiDowngradeUpgradeType(u32, allocator, tc, 123, target_ms);
        try writeArcRowNoVariant(w1, tc, MAX_TOTAL_ITERS, r.stats);
    }
    try w1.print("\n", .{});
    try write_md_append(s1.getWritten());

    var b2: [768]u8 = undefined;
    var s2 = std.io.fixedBufferStream(&b2);
    const w2 = s2.writer();
    try w2.print("### Arc - Heap Downgrade + Upgrade Throughput - ([64]u8)\n", .{});
    try w2.print("| Thread(s) |  Iterations | ns/op (median) | Throughput (ops/s) |\n| --- | --- | --- | --- |\n", .{});
    for (threads_list) |tc| {
        const r = try benchMultiDowngradeUpgradeType([64]u8, allocator, tc, [_]u8{7} ** 64, target_ms);
        try writeArcRowNoVariant(w2, tc, MAX_TOTAL_ITERS, r.stats);
    }
    try w2.print("\n", .{});
...
pub fn run_downgrade_upgrade() !void {
    ...
    var scaled_du: f64 = 0;
    const unit_du = scale_rate_f64(&scaled_du, f_thr_du.median);
...
pub fn run_pool_churn() !void {
    ...
    var fb_r: [48]u8 = undefined;
    var fb_p: [48]u8 = undefined;
...
pub fn run_pool_churn_mt_off(thread_count: usize) !void {
    ...
    var sc_off: f64 = 0;
    const u_off = scale_rate_f64(&sc_off, f_ops_off.median);
    var ob: [48]u8 = undefined;
    std.debug.print("ArcPool MT (stats=off) throughput: {s} {s}\n", .{ fmt_f64_commas2(&ob, sc_off), u_off });
}
...
fn burstyMeasureCapacity(
    ...
    while (cyc < pilot_cycles) : (cyc += 1) {
        ...
        var j: usize = 0;
        while (j < burst) : (j += 1) list[j] = try pool.create(payload);
        j = 0;
        while (j < burst) : (j += 1) pool.recycle(list[j]);
    }
    const pilot_ns = timer.read();
    ...
    while (rep < repeats) : (rep += 1) {
        ...
        while (cyc < run_cycles) : (cyc += 1) {
            ...
            defer std.heap.page_allocator.free(list);
            var k: usize = 0;
            while (k < burst) : (k += 1) list[k] = try pool.create(payload);
            k = 0;
            while (k < burst) : (k += 1) pool.recycle(list[k]);
        }
        const ns = timer.read();
...
pub fn run_pool_stats_toggle() !void {
    ...
    var tb1: [48]u8 = undefined;
    var tb2: [48]u8 = undefined;
...
pub fn run_pool_cyclic_init() !void {
    ...
    const ctor = struct {
        fn f(w: beam.Libs.ArcWeak(PoolPayload)) anyerror!PoolPayload {
    ...
    var sc_c: f64 = 0;
    const u_c = scale_rate_f64(&sc_c, f_thr_cyc.median);
    var cb: [48]u8 = undefined;
    std.debug.print("createCyclic throughput: {s} {s}\n", .{ fmt_f64_commas2(&cb, sc_c), u_c });
}
...
pub fn run_pool_inplace_vs_copy(thread_count: usize) !void {
    ...
    var fx1: [48]u8 = undefined;
    var fx2: [48]u8 = undefined;
...
pub fn run_pool_tls_capacity_compare() !void {
    ...
    var tb16: [48]u8 = undefined;
    var tb32: [48]u8 = undefined;
    var tb64: [48]u8 = undefined;
    ...
    var bb8: [48]u8 = undefined;
    var bb16: [48]u8 = undefined;
    var bb32: [48]u8 = undefined;
    var bb64: [48]u8 = undefined;
    ...
    var nb8: [48]u8 = undefined;
    var nb16: [48]u8 = undefined;
    var nb32: [48]u8 = undefined;
    var nb64: [48]u8 = undefined;
...
pub fn run_pool_split_scenarios() !void {
    ...
    var sb1: [48]u8 = undefined;
    var sb2: [48]u8 = undefined;
    var sb3: [48]u8 = undefined;
...
fn run_uncontended_arc_svo_max() !void {
    const allocator = getAllocator();
    const ArcType = beam.Libs.Arc(u32);
    ...
    const s_iters = fmt_u64_commas(&ib, max_iter);
    var fb1: [48]u8 = undefined;
    const s_ns = fmt_f64_commas2(&fb1, ns_per_f);
    var scaled: f64 = 0;
    const unit = scale_rate_f64(&scaled, ops_s_f);
    var fb2: [48]u8 = undefined;
    const s_rate = fmt_f64_commas2(&fb2, scaled);
