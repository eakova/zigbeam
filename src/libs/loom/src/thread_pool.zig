...
pub fn ThreadPoolFn(comptime config: Config) type {
    ...
    return struct {
            ...
            fn run(self: *Worker) void {
                ...
                while (!self.shutdown.load(.acquire)) {
                    if (self.channel_worker.recv()) |task| {
                        ...
                        defer _ = self.pool().active_tasks.fetchSub(1, .acq_rel);
        ...
        /// Issue 57 fix: Thread count from init_config (default: full CPU count).
        ///
        ...
        pub fn init(allocator: Allocator, init_config: InitConfig) !*Self {
            ...
            // Issue 57 fix: Use full CPU count instead of half for better throughput
            const raw_threads = init_config.num_threads orelse (std.Thread.getCpuCount() catch 2);
            ...
            if (current_worker_ptr) |worker_ptr| {
                ...
                if (worker.pool_ptr == @as(*anyopaque, @ptrCast(self))) {
                    if (worker.channel_worker.recv()) |task| {
                        // Issue 11 fix: Decrement active_tasks after task completes (or panics)
                        defer _ = self.active_tasks.fetchSub(1, .acq_rel);
