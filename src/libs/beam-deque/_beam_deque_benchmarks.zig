...
fn benchOwnerPush() !void {
    ...
    var result = try BeamDeque(u64).init(allocator, 4096);
...
fn benchConcurrentSteal() !void {
    ...
    var result = try BeamDeque(u64).init(allocator, 8192);
    defer result.worker.deinit();
    ...
    var thieves: [8]Thread = undefined;
    for (&thieves) |*thief| {
...
fn benchLastItemRace() !void {
    ...
    var result = try BeamDeque(u64).init(allocator, 8);
    defer result.worker.deinit();
    ...
    while (i < iterations) : (i += 1) {
        ...
        const thief = try Thread.spawn(.{}, struct {
            fn run(stealer: *BeamDeque(u64).Stealer) void {
                _ = stealer.steal();
            }
        }.run, .{&result.stealer});
...
pub fn main() !void {
    std.debug.print("\n╔═══════════════════════════════════════════════════╗\n", .{});
    std.debug.print("║      BeamDeque Performance Benchmarks            ║\n", .{});
    std.debug.print("╚═══════════════════════════════════════════════════╝\n", .{});
