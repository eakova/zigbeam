...
/// BeamDequeChannel - High-Performance MPMC Channel with Work-Stealing
///
/// Architecture:
/// - Local deques: Each worker has a private bounded BeamDeque (fast path)
...
pub fn BeamDequeChannel(
    ...
    comptime {
        ...
        if (size > std.atomic.cache_line and !is_pointer) {
            @compileError(std.fmt.comptimePrint(
                "BeamDequeChannel: Type '{s}' ({d} bytes) exceeds cache line size ({d} bytes). " ++
    ...
    return struct {
        ...
        const PaddedStealer = struct {
            ...
            _padding: [std.atomic.cache_line - @sizeOf(BeamDeque(T).Stealer)]u8 = undefined,
        };
        ...
        /// Contains the BeamDeque worker, worker ID, RNG, and channel reference
        pub const Worker = struct {
            deque_worker: BeamDeque(T).Worker,
            worker_id: usize,
        ...
        pub fn init(allocator: Allocator, num_workers: usize) !InitResult {
            ...
            while (i < num_workers) : (i += 1) {
                const result = try BeamDeque(T).init(allocator, local_capacity);
                workers[i] = Worker{
                    .deque_worker = result.worker,
                    .worker_id = i,
                    // Initialize RNG with worker_id as seed for deterministic but unique sequences
                    .rng = std.Random.DefaultPrng.init(i),
                    .channel = channel,
                };
        ...
        fn handleOffload(self: *Self, worker: *Worker, item: T) !void {
            ...
            const target_offload = worker.capacity() / 2;
