...
const Deque = @import("beam-deque").Deque;

// =============================================================================
// Single-Threaded Correctness Tests
// =============================================================================

test "Deque: init with valid capacity" {
...
test "Deque: init rejects non-power-of-two capacity" {
    try testing.expectError(error.CapacityNotPowerOfTwo, Deque(u32).init(testing.allocator, 15));
    try testing.expectError(error.CapacityNotPowerOfTwo, Deque(u32).init(testing.allocator, 17));
    try testing.expectError(error.CapacityNotPowerOfTwo, Deque(u32).init(testing.allocator, 100));
}

test "Deque: basic push and pop (LIFO)" {
...
test "Deque: mixed pop and steal" {
    var result = try Deque(u32).init(testing.allocator, 8);
    defer result.worker.deinit();
...
test "Deque: capacity enforcement (error.Full)" {
    var result = try Deque(u32).init(testing.allocator, 4);
    defer result.worker.deinit();
...
test "Deque: empty steal returns null" {
    var result = try Deque(u32).init(testing.allocator, 8);
    defer result.worker.deinit();
...
test "Deque: wraparound behavior" {
    var result = try Deque(u32).init(testing.allocator, 4);
    defer result.worker.deinit();
...
test "Deque: single item race simulation (owner wins)" {
    var result = try Deque(u32).init(testing.allocator, 8);
    defer result.worker.deinit();
...
test "Deque: works with different types" {
    // Test with struct
    const Task = struct {
        id: u64,
        priority: u8,
    ...
    var result = try Deque(Task).init(testing.allocator, 8);
    defer result.worker.deinit();
...
test "Deque: large capacity" {
    var result = try Deque(u64).init(testing.allocator, 1024);
    defer result.worker.deinit();
