...
test "BeamDeque accepts u64" {
    const allocator = std.testing.allocator;
    var result = try BeamDeque(u64).init(allocator, 64);
...
test "BeamDeque accepts pointer types" {
    const LargeType = struct {
        data: [128]u8,
    };

    const allocator = std.testing.allocator;
    var result = try BeamDeque(*LargeType).init(allocator, 64);
    defer result.worker.deinit();
...
test "BeamDeque accepts small structs" {
    ...
    const allocator = std.testing.allocator;
    var result = try BeamDeque(SmallStruct).init(allocator, 64);
    defer result.worker.deinit();
...
//
// test "BeamDeque rejects large types" {
//     const LargeType = struct {
//         data: [128]u8,
//     };
//
...
//
// Expected error:
// BeamDeque: Type '_test_fat_type_validation.test.BeamDeque rejects large types.LargeType'
// (128 bytes) exceeds cache line size (64 bytes). Use *_test_fat_type_validation.test.BeamDeque
