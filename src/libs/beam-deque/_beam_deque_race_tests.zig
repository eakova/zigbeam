...
test "BeamDeque: single item race (owner pop vs thief steal)" {
    ...
    while (i < iterations) : (i += 1) {
        ...
        const thief_handle = try Thread.spawn(.{}, struct {
            fn run(stealer: *BeamDeque(u32).Stealer) void {
                _ = stealer.steal();
            }
        }.run, .{&result.stealer});
...
test "BeamDeque: concurrent push and steal" {
    var result = try BeamDeque(usize).init(testing.allocator, 1024);
    defer result.worker.deinit();
    ...
    for (&thieves) |*thief| {
        thief.* = try Thread.spawn(.{}, struct {
            fn run(stealer: *BeamDeque(usize).Stealer, counter: *Atomic(usize)) void {
                var count: usize = 0;
                while (true) {
                    if (stealer.steal()) |_| {
                        count += 1;
...
test "BeamDeque: multi-threaded stress test (1M items)" {
    var result = try BeamDeque(u64).init(testing.allocator, 4096);
    defer result.worker.deinit();
    ...
    for (&thieves) |*thief| {
        thief.* = try Thread.spawn(.{}, struct {
            fn run(stealer: *BeamDeque(u64).Stealer, counter: *Atomic(usize), items: []Atomic(bool)) void {
                var count: usize = 0;
                while (true) {
...
test "BeamDeque: multiple thieves contention" {
    var result = try BeamDeque(u32).init(testing.allocator, 1024);
    defer result.worker.deinit();
    ...
    for (&thieves, 0..) |*thief, idx| {
        thief.* = try Thread.spawn(.{}, struct {
            fn run(stealer: *BeamDeque(u32).Stealer, counter: *Atomic(usize)) void {
                var count: usize = 0;
...
test "BeamDeque: multiple thieves contention-128" {
    var result = try BeamDeque(u32).init(testing.allocator, 128);
    defer result.worker.deinit();
    ...
    for (&thieves, 0..) |*thief, idx| {
        thief.* = try Thread.spawn(.{}, struct {
            fn run(stealer: *BeamDeque(u32).Stealer, counter: *Atomic(usize), done_flag: *Atomic(bool)) void {
                var count: usize = 0;
...
test "BeamDeque: concurrent push/pop by owner, steal by thieves" {
    var result = try BeamDeque(u64).init(testing.allocator, 256);
    defer result.worker.deinit();
    ...
    for (&thieves) |*thief| {
        thief.* = try Thread.spawn(.{}, struct {
            fn run(stealer: *BeamDeque(u64).Stealer, counter: *Atomic(usize), done_flag: *Atomic(bool)) void {
                var count: usize = 0;
                while (!done_flag.load(.acquire) or !stealer.isEmpty()) {
                    if (stealer.steal()) |_| {
                        count += 1;
...
test "BeamDeque: wraparound with concurrent access" {
