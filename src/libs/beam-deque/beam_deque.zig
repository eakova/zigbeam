...
/// BeamDeque - A Bounded, High-Performance Work-Stealing Deque
///
/// A bounded SPMC work-stealing deque based on Arora-Blumofe-Plaxton principles.
/// NOTE: This is NOT the dynamic Chase-Lev algorithm - this is a fixed-capacity variant.
/// This is a Single-Producer, Multi-Consumer (SPMC) bounded deque.
///
/// Architecture:
...
pub fn BeamDeque(comptime T: type) type {
    ...
    comptime {
        ...
        if (size > std.atomic.cache_line and !is_pointer) {
            @compileError(std.fmt.comptimePrint(
                "BeamDeque: Type '{s}' ({d} bytes) exceeds cache line size ({d} bytes). " ++
    ...
    return struct {
        ...
        pub fn init(allocator: Allocator, capacity: usize) !InitResult {
            ...
            const deque = try allocator.create(Deque);
