...
// =============================================================================
// BeamDequeChannel Tests
// =============================================================================

test "BeamDequeChannel: basic send and recv" {
    const Channel = BeamDequeChannel(u32, 256, 1024);
...
test "BeamDequeChannel: work stealing between workers" {
    const Channel = BeamDequeChannel(u64, 256, 1024);
    var result = try Channel.init(testing.allocator, 4);
    defer result.channel.deinit(&result.workers);
...
test "BeamDequeChannel: overflow to global queue" {
    const Channel = BeamDequeChannel(u32, 8, 64); // Small local capacity
...
test "BeamDequeChannel: back-pressure when full" {
    const Channel = BeamDequeChannel(u32, 4, 8); // Very small capacities
...
test "BeamDequeChannel: 8P/8C stress test (1M items)" {
    const Channel = BeamDequeChannel(u64, 256, 4096);
...
test "BeamDequeChannel: 1P/8C load balancing test" {
    const Channel = BeamDequeChannel(u64, 128, 2048);
...
test "BeamDequeChannel: recv priority ordering" {
    const Channel = BeamDequeChannel(u32, 16, 64);
...
test "BeamDequeChannel: no self-stealing" {
    const Channel = BeamDequeChannel(u64, 256, 1024);
    var result = try Channel.init(testing.allocator, 1);
    defer result.channel.deinit(&result.workers);
