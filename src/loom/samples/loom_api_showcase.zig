...
pub fn main() !void {
    ...
    // Use u64 for sum to avoid overflow with 10M entries
    const response_times_u64 = try par_iter(response_times_for_reduce).withPool(pool).map(u64, struct {
        fn toU64(val: u32) u64 {
            return @as(u64, val);
        }
    }.toU64, allocator);
    defer allocator.free(response_times_u64);
    const sum_response = par_iter(response_times_u64).withPool(pool).reduce(Reducer(u64).sum());
    elapsed = timer.read();

    const throughput = @as(f64, @floatFromInt(response_times_for_reduce.len)) / (@as(f64, @floatFromInt(elapsed)) / 1_000_000_000.0);
    std.debug.print("LOOM PERFORMANCE: {d:.2}ms to reduce 10M entries\n", .{
        @as(f64, @floatFromInt(elapsed)) / 1_000_000.0,
    });
    std.debug.print("Throughput: {d:.2}M ops/sec\n", .{throughput / 1_000_000.0});
    std.debug.print("Results (sample data values, not performance):\n", .{});
    std.debug.print("  Max: {d}, Min: {d}, Avg: {d}\n\n", .{
        max_response,
        min_response,
        sum_response / @as(u64, response_times_for_reduce.len),
    });
