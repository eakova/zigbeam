// par_iter.zig - Parallel Iterator Operations
//
...
const stepped_iterator = @import("stepped_iter.zig");
...
// Standalone operations module
pub const ops = @import("../ops/ops.zig");

// Range iterator module
const range_mod = @import("par_range.zig");
pub const RangeParallelIterator = range_mod.RangeParallelIterator;
pub const ContextRangeParallelIterator = range_mod.ContextRangeParallelIterator;
pub const par_range = range_mod.par_range;
...
pub fn ParallelIterator(comptime T: type) type {
    return struct {
        ...
        pub fn forEach(self: Self, comptime func: fn (*T) void) void {
            ...
            // Stack fallback uses stack_max_chunks (32), heap allows up to heap_max_chunks (128)
            const max_chunks = if (self.allocator != null)
            ...
            const heap_contexts = allocContextsIfNeeded(Context, self.allocator, spawn_chunks) catch {
        ...
        pub fn filter(self: Self, comptime pred: fn (T) bool, alloc_override: ?Allocator) ![]T {
            ...
            // When chunk_idx can reach stack_max_chunks-1 (31), we access counts[32], so need size 33
