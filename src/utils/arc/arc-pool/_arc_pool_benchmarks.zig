//! ArcPool-only benchmarks. Writes report to docs/utils/arcpool_benchmark_results.md
//! How to run:
...
const PoolTypeOff = beam.Utils.ArcPool(PoolPayload, false);

const MD_PATH = "docs/utils/arcpool_benchmark_results.md";
...
fn fmt_u64_commas(buf: *[32]u8, value: u64) []const u8 {
    var i: usize = buf.len;
    var v = value;
    if (v == 0) { i -= 1; buf[i] = '0'; return buf[i..]; }
    var group: u32 = 0;
    while (v > 0) {
        ...
        i -= 1; buf[i] = '0' + d;
        group += 1;
        if (v != 0 and group % 3 == 0) { i -= 1; buf[i] = ','; }
    }
    return buf[i..];
}
...
fn sortAsc(slice: []u64) void { var i: usize = 1; while (i < slice.len) : (i += 1) { var j: usize = i; while (j > 0 and slice[j-1] > slice[j]) : (j -= 1) { const t = slice[j-1]; slice[j-1]=slice[j]; slice[j]=t; } } }
fn medianIqr(slice_in: []const u64) struct { median: u64, q1: u64, q3: u64 } { var b: [16]u64 = .{0} ** 16; const n = slice_in.len; if (n==0) return .{.median=0,.q1=0,.q3=0}; @memcpy(b[0..n], slice_in); sortAsc(b[0..n]); const mid=n/2; const med= if(n%2==1) b[mid] else (b[mid-1]+b[mid])/2; return .{ .median=med, .q1=b[n/4], .q3=b[(3*n)/4] }; }
fn ratio_ops_per_sec(ns: u64, ops: u64) f64 { if (ns==0) return 0; return (@as(f64,@floatFromInt(ops))*1_000_000_000.0)/@as(f64,@floatFromInt(ns)); }
fn ratio_ns_per_op(ns: u64, ops: u64) f64 { if (ops==0) return 0; return @as(f64,@floatFromInt(ns))/@as(f64,@floatFromInt(ops)); }
fn medianIqrOpsPerSecF64(stats: *const Stats) f64 { if (stats.len==0) return 0; var b:[16]f64=. {0} ** 16; var i:usize=0; while(i<stats.len): (i+=1) b[i]=ratio_ops_per_sec(stats.ns_list[i], stats.ops_list[i]); // insertion sort
    var k:usize=1; while(k<stats.len): (k+=1) { var j:usize=k; while(j>0 and b[j-1]>b[j]) : (j-=1) { const t=b[j-1]; b[j-1]=b[j]; b[j]=t; } } const mid=stats.len/2; return if(stats.len%2==1) b[mid] else (b[mid-1]+b[mid])/2.0; }
fn medianIqrNsPerOpF64(stats: *const Stats) f64 { if (stats.len==0) return 0; var b:[16]f64=. {0} ** 16; var i:usize=0; while(i<stats.len): (i+=1) b[i]=ratio_ns_per_op(stats.ns_list[i], stats.ops_list[i]); var k:usize=1; while(k<stats.len): (k+=1) { var j:usize=k; while(j>0 and b[j-1]>b[j]) : (j-=1) { const t=b[j-1]; b[j-1]=b[j]; b[j]=t; } } const mid=stats.len/2; return if(stats.len%2==1) b[mid] else (b[mid-1]+b[mid])/2.0; }

fn record(stats: *Stats, ops: u64, ns: u64) void { if (stats.len>=stats.ns_list.len) return; stats.ns_list[stats.len]=ns; stats.ops_list[stats.len]=ops; stats.len+=1; }
...
fn benchPoolChurnST(value: [64]u8) !struct { stats: Stats, iterations: u64 } {
    ...
    var stats = Stats{}; record(&stats, run_iters, t.read());
...
fn benchPoolChurnMT(threads: usize, per_thread_iters: u64) !struct { stats: Stats, iterations: u64 } {
    ...
    const Worker = struct {
        fn run(ctx: *Ctx) void {
            ...
            while (ctx.start.load(.seq_cst) == 0) { std.Thread.yield() catch {}; }
    ...
    const contexts = try allocator.alloc(Ctx, threads); defer allocator.free(contexts);
    var start_flag = std.atomic.Value(usize).init(0);
    var k: usize = 0; while (k < threads) : (k += 1) { contexts[k] = .{ .pool = &pool, .iterations = per_thread_iters, .start = &start_flag }; handles[k] = try Thread.spawn(.{}, Worker.run, .{ &contexts[k] }); }
    var t = try Timer.start(); _ = start_flag.store(1, .seq_cst); for (handles) |h| h.join(); const ns = t.read();
    pool.drainThreadCache(); var stats = Stats{}; record(&stats, total_iters, ns); return .{ .stats = stats, .iterations = total_iters };
}
...
fn measurePoolChurnMTOff(pool: *PoolTypeOff, allocator: std.mem.Allocator, thread_count: usize, iterations: u64) !u64 {
    ...
    const handles = try allocator.alloc(Thread, thread_count); defer allocator.free(handles);
    const Ctx = struct { pool: *PoolTypeOff, iterations: u64, warmups: usize, start_flag: *std.atomic.Value(usize) };
    const contexts = try allocator.alloc(Ctx, thread_count); defer allocator.free(contexts);
    var start_flag = std.atomic.Value(usize).init(0);
    const Worker = struct {
        fn run(ctx: *Ctx) void {
            var i: u64 = 0;
...
fn writeBenchRow(wr: anytype, label: []const u8, iterations: u64, stats: Stats) !void {
    var nb1: [32]u8 = undefined; const s_iters = fmt_u64_commas(&nb1, iterations);
    const f_ops = medianIqrOpsPerSecF64(&stats);
    const f_ns = medianIqrNsPerOpF64(&stats);
    // Format throughput
    var sc: f64 = 0; const unit = scale_rate_f64(&sc, f_ops);
    var fb_rate: [48]u8 = undefined; const s_rate = fmt_f64_commas2(&fb_rate, sc);
...
pub fn main() !void {
    ...
    // 1) ST Create + Recycle
    {
        ...
        var b: [512]u8 = undefined; var s = std.io.fixedBufferStream(&b); const w = s.writer();
        try w.print("### ArcPool - Single-Threaded - Create + Recycle - ([64]u8)\n", .{});
        try w.print("| Thread(s) | Iterations | ns/op (median) | Throughput (ops/s) |\n| --- | --- | --- | --- |\n", .{});
        var nb: [32]u8 = undefined; const s_thr = fmt_u64_commas(&nb, 1);
        var row: [256]u8 = undefined; var rs = std.io.fixedBufferStream(&row);
    ...
    // 2) MT Create + Recycle
    {
        ...
        for (threads_list) |tc| {
            ...
            var nb: [32]u8 = undefined; const s_thr = fmt_u64_commas(&nb, tc);
            var row: [256]u8 = undefined; var rs = std.io.fixedBufferStream(&row);
    ...
    // 3) TLS Capacity (ST)
    {
        var gpa = std.heap.GeneralPurposeAllocator(.{}){}; defer _ = gpa.deinit();
        const allocator = gpa.allocator();
        var p16 = PoolTypeOff.init(allocator, .{ .logical_cpus = null }); p16.tls_active_capacity = 16; defer p16.deinit();
        var p32 = PoolTypeOff.init(allocator, .{ .logical_cpus = null }); p32.tls_active_capacity = 32; defer p32.deinit();
        var p64 = PoolTypeOff.init(allocator, .{ .logical_cpus = null }); p64.tls_active_capacity = 64; defer p64.deinit();
    ...
    // 4) In-place vs Copy (ST)
    {
        ...
        const run_iters: u64 = MAX_TOTAL_ITERS;
        var t = try Timer.start(); var i: u64 = 0; while (i < run_iters) : (i += 1) { const arc = try pool.createWithInitializer(struct { fn init(p: *[64]u8) void { @memset(p, 5); } }.init); pool.recycle(arc); }
        pool.drainThreadCache();
        var inplace = Stats{}; record(&inplace, run_iters, t.read());
        pool.deinit();
        var b: [512]u8 = undefined; var s = std.io.fixedBufferStream(&b); const w = s.writer();
        ...
        var nb: [32]u8 = undefined; const s_thr = fmt_u64_commas(&nb, 1);
        var nbx: [32]u8 = undefined; const s_iters_c = fmt_u64_commas(&nbx, copy.iterations);
        const f_ops_c = medianIqrOpsPerSecF64(&copy.stats); var sc_c: f64 = 0; const unit_c = scale_rate_f64(&sc_c, f_ops_c);
        const f_ns_c = medianIqrNsPerOpF64(&copy.stats); var fb1: [48]u8 = undefined; const s_rate_c = fmt_f64_commas2(&fb1, sc_c);
        ...
        var nby: [32]u8 = undefined; const s_iters_ip = fmt_u64_commas(&nby, run_iters);
        const f_ops_ip = medianIqrOpsPerSecF64(&inplace); var sc_ip: f64 = 0; const unit_ip = scale_rate_f64(&sc_ip, f_ops_ip);
        const f_ns_ip = medianIqrNsPerOpF64(&inplace); var fb2: [48]u8 = undefined; const s_rate_ip = fmt_f64_commas2(&fb2, sc_ip);
